...
''')
        ...
        self.paths = (path / '.'.join([self.names[0], self.ext]),
                      self.pkg / '.'.join([self.names[1], self.ext]),
                      self.pkg / '.'.join([self.names[2], self.ext]),
                      self.pkg / '.'.join([self.names[3], self.ext]))
        self.msg = "Created package {} in {}".format(self.names[2],
                                                     path.as_posix())
...
def main():
    ...
    if args.command == "new":
        path = pathlib.Path().cwd() / args.name
        if path.exists():
